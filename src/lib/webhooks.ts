...
export const handleStripeWebhook = async (event: Stripe.Event) => {
  try {
    if (event.type === "checkout.session.completed") {
      ...
      const { cartId, userId, guestUserId, checkoutSessionId } =
        session.metadata;
      ...
      const shippingAddress = {
        ...
        lastName:
          collectedInformation?.name?.split(" ").slice(1).join(" ") ||
          "Customer",
      ...
      if (printfulOrderResult.status === "SUCCESS") {
        ...
        await prisma.order.update({
          ...
          data: {
            printfulOrderId:
              printfulOrderResult.data?.printfulOrderId.toString(),
        ...
        await prisma.printfulOrder.create({
          data: {
            ...
            printfulOrderId:
              printfulOrderResult.data?.printfulOrderId.toString() || "",
            printfulStatus:
              printfulOrderResult.data?.printfulStatus || "unknown",
      ...
      await sendOrderConfirmationEmailFromOrder(
        order,
