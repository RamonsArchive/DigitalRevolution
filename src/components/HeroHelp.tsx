...
const HeroHelp = ({
  ...
  return (
    <div className="flex flex-col gap-16 md:gap-24 py-32 px-5 md:px-20 bg-linear-to-br bg-primary-200 min-h-screen">
      {/* Main Title Section */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full">
        {description.map((item, index) => (
          <a
            ...
            key={`support-${index}`}
            className="help-card group flex flex-col items-center justify-center p-8 rounded-3xl bg-linear-to-br from-secondary-800/80 to-primary-800/80 border border-secondary-600/40 shadow-xl hover:shadow-2xl transition-all duration-500 hover:from-secondary-700/80 hover:to-primary-700/80 hover:scale-105 hover:-translate-y-2"
          >
            {/* Icon placeholder - you can add icons later */}
            <div className="w-16 h-16 mb-6 rounded-full bg-linear-to-br from-primary-500 to-secondary-500 flex items-center justify-center shadow-lg group-hover:shadow-primary-500/50 transition-all duration-300">
            ...
            {/* Hover effect line */}
            <div className="w-0 h-1 bg-linear-to-r from-primary-400 to-secondary-400 mt-4 group-hover:w-full transition-all duration-500 rounded-full"></div>
          </a>
        ))}
      </div>
